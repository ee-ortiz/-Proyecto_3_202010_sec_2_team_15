...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
	...
	public Iterable<Key> keysToIterate() {
		...
		return keysInRangeToIterate(min(), max());
	}

	/**
	 * Retorna todas llaves del árbol como un iterador
	 * Se retornan inOrden
	 * @return all keys in the symbol table as an iterator
	 */
	public Iterator<Key> keys() {

		Iterator<Key> rta = keysToIterate().iterator();
		return rta;
	}
	 ...
	 *    is {@code null}
	 */
	public Iterable<Key> keysInRangeToIterate(Key lo, Key hi) {
	...
	private Iterable<Node> HojasIterables(Key lo, Key hi) { 
		if (lo == null) throw new IllegalArgumentException("first argument to keys() is null");
		if (hi == null) throw new IllegalArgumentException("second argument to keys() is null");
	...
	// Iterador que devuelve todas las hojas del arbol de forma ordenada de menor a mayor
	public Iterator<Node> IteratorDeHojas(){

		return HojasIterables(min(), max()).iterator();

	}


	private void nodes(Node x, Queue<Node> queue, Key lo, Key hi) { 
		if (x == null) return; 
		int cmplo = lo.compareTo(x.key); 
		int cmphi = hi.compareTo(x.key); 
		if (cmplo < 0) nodes(x.left, queue, lo, hi); 
		if (cmplo <= 0 && cmphi >= 0 && x.esHoja()) queue.enqueue(x); 
		if (cmphi > 0) nodes(x.right, queue, lo, hi); 

	} 

	public Iterator<Key> keysInRange(Key lo, Key hi){

		return keysInRangeToIterate(lo, hi).iterator();
	}
